    ...
    if (loading) {
        return (
            <header className="flex items-center justify-between py-2.5 px-6 bg-white border-b border-gray-100 shadow-sm z-30 min-h-[56px]">
                <div className="h-4 w-32 bg-gray-100 rounded animate-pulse lg:hidden"></div>
                <h2 className="text-xs font-black text-gray-900 hidden sm:block uppercase tracking-widest opacity-60">Dashboard Analitik</h2>
    ...
    if (!adminData) {
        return (
            <header className="flex items-center justify-end p-6 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border-b border-slate-700/50 shadow-lg backdrop-blur-sm">
                <p className="text-red-400 mr-4 text-sm font-bold uppercase tracking-tight">Data Admin tidak ditemukan.</p>
                <Link href="/login" className="px-4 py-2 text-xs font-black text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-all uppercase tracking-widest">
                    Login Ulang
                </Link>
            </header>
        );
        ...
        <header className="flex items-center justify-between py-2.5 px-6 bg-white border-b border-gray-100 shadow-sm z-30 min-h-[56px]">
            ...
            <div className="flex items-center space-x-3">
                <button className="p-1.5 text-gray-400 rounded-lg hover:bg-gray-50 relative transition-colors">
                    ...
                    <span className="absolute top-0 right-0 text-[7px] font-black bg-[#0138C9] text-white px-1 py-0.5 rounded-full border border-white">3</span>
                </button>

                <div className="flex items-center gap-3 pl-3 border-l border-gray-100">
                    <div className="text-right hidden sm:block">
                        <p className="text-[10px] font-black text-gray-900 uppercase tracking-tight leading-none">Hi, {nameAdmin}</p>
                        <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">{roleAdmin}</p>
                    </div>
