...
export default function AdminBidsPage() {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case "approved":
                return { bg: "bg-green-500/10", text: "text-green-500", label: "Disetujui" };
            case "rejected":
                return { bg: "bg-red-500/10", text: "text-red-500", label: "Ditolak" };
            case "pending":
            default:
                return { bg: "bg-yellow-500/10", text: "text-yellow-500", label: "Menunggu" };
        }
    };

    return (
        <main className="flex-1 p-6 min-h-screen bg-[#0f172a] text-white">
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan={7} className="text-center py-8 text-gray-400 uppercase font-black text-[10px] tracking-widest">Tidak ada record bid.</td>
                                    </tr>
                                ) : (
                                        ...
                                        return (
                                            <tr key={bid.id} className="hover:bg-white/[0.02] transition-colors group">
                                                ...
                                                <td className="py-3 px-4 text-center">
                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border ${statusColor.bg} ${statusColor.text} border-transparent`}>
                                                        {statusColor.label}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="py-3 px-4 text-center whitespace-nowrap">
                                                    {bid.status === "pending" ? (
                                                        ...
                                                        <span className="text-xs text-gray-400 uppercase font-black opacity-30">—</span>
                                                    )}
                                                </td>
                                            </tr>
                                        );
